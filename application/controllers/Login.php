		...
		if($status['status']=='1'){
			...
			if($this->config->item('wp-login')){
		                ...
		                if($this->authentication($username, $password)){
		                 ...
		                 $this->load->helper('url');
		                 $this->session->set_flashdata('message', "<div class='d-grid mb-2'> <button class='btn  btn-info text-uppercase fw-bold' > ".$status['message']."   <i class='fa fa-exclamation-triangle'></i></button>
	 </div>");
			 ...
			 header("location:$burl");
		        ...
		        $this->load->helper('url');
		        $this->session->set_flashdata('message', "<div class='d-grid mb-2'> <button class='btn  btn-danger text-uppercase fw-bold' >".$status['message']."   <i class='fa fa-exclamation-circle'></i> </button>
	 </div>");
			redirect('login');
		        }
			...
			// email not verified
			$this->session->set_flashdata('message', "<div class='d-grid mb-2'> <button class='btn  btn-info text-uppercase fw-bold'>".$status['message']."     <i class='fa fa-envelope-open-text'></i> </button>
	 </div>");
			redirect('login');
			...
			// email not verified
			$this->session->set_flashdata('message', "<div class='d-grid mb-2'> <button class='btn  btn-warning text-uppercase fw-bold'>".$status['message']."    <i class='fa fa-exclamation-triangle'></i> </button>
	 </div>");
			redirect('login');
		}
	...
	function verify($vcode){
		$this->load->helper('url');	 
		 if($this->user_model->verify_code($vcode)){
				$this->session->set_flashdata('message', "<div class='d-grid mb-2'> <button class='btn  btn-success text-uppercase fw-bold'>".$this->lang->line('email_verified')." </div>");
        
         redirect('login');

			}else{
				$this->session->set_flashdata('message', "<div class='d-grid mb-2'> <button class='btn  btn-danger text-uppercase fw-bold'>".$this->lang->line('invalid_otp')."    <i class='fas fa-exclamation-triangle'></i> </button>
	 </div>");
	...
	$this->load->helper('url');
			...
			$user_email=$this->input->post('email');
			 if($this->user_model->reset_password($user_email)){
				$this->session->set_flashdata('message', "<div class='d-grid mb-2'> <button class='btn  btn-success text-uppercase fw-bold'>".$this->lang->line('password_updated')." </div>");
						
			}else{
				$this->session->set_flashdata('message', "<div class='d-grid mb-2'> <button class='btn  btn-danger text-uppercase fw-bold'>".$this->lang->line('email_doesnot_exist')." </div>");
          ...
          if ($this->form_validation->run() == FALSE)
                ...
                else
                {
					if($this->user_model->insert_user_2()){
                         $userdata['email']=$mailing; 
                        if($this->config->item('verify_email')){
						$this->session->set_flashdata('message', "<div class='d-grid mb-2'> <button class='btn  btn-info text-uppercase fw-bold' type='submit'>".$this->lang->line('account_registered_email_sent').$mailing."     <i class='fa fa-envelope-open-text'></i> </button>
	 </div>");
						}else{
							$this->session->set_flashdata('message', "<div class='d-grid mb-2'> <button class='btn  btn-success text-uppercase fw-bold'>".$this->lang->line('account_registered')." </div>");
						}
						}else{
						    $this->session->set_flashdata('message', "<div class='d-grid mb-2'> <button class='btn  btn-danger text-uppercase fw-bold'>".$this->lang->line('error_to_add_data')." </div>");
