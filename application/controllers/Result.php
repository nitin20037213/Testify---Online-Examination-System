...
class Result extends CI_Controller {
	...
	function generate_report(){
		...
		$csvdata=$this->lang->line('roll_no').",".$this->lang->line('full_name').",".$this->lang->line('father_name').",".$this->lang->line('score_obtained').",".$this->lang->line('percentage_obtained').",".$this->lang->line('status')."\r\n";
		foreach($result as $rk => $val){
		$csvdata.=$val['uid'].",".$val['full_name'].",".$val['father_name'].",".$val['score_obtained'].",".$val['percentage_obtained'].",".$val['result_status']."\r\n";
		}
		$filename=$val['quiz_name'].'.csv';
	...
	$position = 1;
	$value=array();
     $value[]=array('Users','Percentage (%)','Position');
     foreach($last_ten_result as $val){
     $value[]=array($val['full_name'],intval($val['percentage_obtained']),$position++);
     }
